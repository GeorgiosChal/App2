...
export default class RegistrationForm{
    ...
    isDifferent(element,handler){
        if(element.previousValue!=element.value){
            //call handler function as it was called from "this" object
            handler.call(this)
        }
    ...
    emailAfterDelay(){
        if(!/^\S+@\S+$/.test(this.email.value)){
            this.showValidationError(this.email,"You must provide an email address")
    ...
    insertValidationElements(){
        this.allFields.forEach(function(element){
            element.insertAdjacentHTML('afterend','<div class="alert alert-danger samll liveValidateMessage"></div>')
        })
    }
